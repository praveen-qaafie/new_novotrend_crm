  ...
  return (
    <div className="relative">
      ...
      <button
        ...
        type="button"
        className="flex w-full items-center justify-between rounded-md border bg-white px-4 py-2.5 hover:bg-gray-50"
      >
        <span className="flex items-center gap-2 text-sm text-gray-800">
          {value ? value.country_name : "Select Country"}
        </span>
        <svg width="14" viewBox="0 0 24 24">
      ...
      {/* Dropdown */}
      {open && (
        <div className="absolute z-[1000] mt-1 max-h-80 w-full overflow-auto rounded bg-white p-2 shadow">
          <input
            placeholder="Search..."
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            className="mb-2 w-full rounded border bg-gray-50 px-3 py-2 text-sm"
          />
          ...
          {isLoading ? (
            ...
            filtered.map((c) => (
              ...
              >
                ...
                <span className="ml-auto text-xs text-gray-400">+{c.country_code}</span>
              </div>
            ))
          )}
        </div>
